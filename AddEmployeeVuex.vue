<template>
  <div class="submitform" id="addEmployee">
    ...
    <div v-if="employee.name.length > 0 && employee.surname.length > 0 && employee.age > 0">
      <button v-on:click="saveEmployee(/*employeesToAdd*/)" class="btn btn-success">Add</button>
    </div>
...
export default {
  ...
  methods: {
    saveEmployee(/*employeesToAdd*/) {
      ...
      employeeService
        .getSaveEmployee(data)
        .then(() => {
          /*  employeesToAdd.push(data);
          this.$store.dispatch("moduleA/popolaStore", employeesToAdd);*/
          this.refresh();
        })
        .catch(e => {
          console.log(e);
